...
def create_app(test_config=None):
  ...
  def quizzes():
    ...
    if category_id == 0:
      question = Question.query.filter(not_(Question.id.in_(previous_questions))).order_by(func.random()).first()

    else:
      question = Question.query.filter(Question.category == str(category_id)).filter(not_(Question.id.in_(previous_questions))).order_by(func.random()).first()
